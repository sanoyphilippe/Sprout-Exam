# Back-end for Sprout App

## Prerequisites
Must have Docker installed

## Env
Create .env file and place the following contents
```
MONGO_INITDB_DATABASE=sprout

DATABASE_URL=mongodb://mongo-server

ACCESS_TOKEN_EXPIRES_IN=1200
REFRESH_TOKEN_EXPIRES_IN=3600
JWT_ALGORITHM=RS256

SUPER_ADMIN_USERNAME=admin
SUPER_ADMIN_PASSWORD='$2a$12$DaCqDnrl7Gbv9qYAgAl2iu8GFdO2yPziesWvUJb2JyEXAC/pkukd2'

CLIENT_ORIGIN=http://localhost:9000

JWT_PRIVATE_KEY=
JWT_PUBLIC_KEY=LS0tLS1CRUdJTiBQVUJMSUMgS0VZLS0tLS0KTUZ3d0RRWUpLb1pJaHZjTkFRRUJCUUFEU3dBd1NBSkJBSSs3QnZUS0FWdHVQYzEzbEFkVk94TlVmcWxzMm1SVgppUFpSclRaY3d5eEVYVURqTWhWbi9KVHRsd3h2a281T0pBQ1k3dVE0T09wODdiM3NOU3ZNd2xNQ0F3RUFBUT09Ci0tLS0tRU5EIFBVQkxJQyBLRVktLS0tLQ==

```

## How to Run the app:
Run Command

``docker-compose up --build``

## API Documentation
Once the application is running please see

``http://localhost:8000/docs``

## Access Credentials
username: admin
password: password
